...
class Send_System_Info_Plugin {
	...
	static function enqueue() {
		wp_register_script( 'send-system-info-script', plugins_url( 'send-system-info.js', __FILE__ ), array( 'jquery' ) );
	...
	static function render_page() {
		...
		<div class="wrap">
			<h2 style="margin-bottom: 1em;"><?php _e( 'Send System Info', 'send-system-info' ); ?></h2>
				<div id="templateside">
					<?php do_action( '' ); ?>
					<p style="margin-top: 0"><?php _e( 'Send System Info displays...', '' ) ?></p>
					<?php do_action( '' ); ?>
				</div>
				<div id="template">
...
<?php self::display() ?>
					...
					<h3><?php _e( 'Send via Email', 'send-system-info' ) ?></h3>
					<?php self::email_form() ?>
					<h3><?php _e( 'Remote Viewing', 'send-system-info' ) ?></h3>
...
self::display_output( $browser, $theme_data, $theme, $host, $WP_REMOTE_POST, $return );

	}

	//Render Info Display
	static function display_output( $browser, $theme_data, $theme, $host, $WP_REMOTE_POST, $return ) {
		global $wpdb;
		ob_start(); ?>
